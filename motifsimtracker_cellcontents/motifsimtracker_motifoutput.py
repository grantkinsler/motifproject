import csv
from motifsimtracker_trial import motifsimtracker_trial
import itertools
import numpy
...
def motifsimtracker_motifoutput(celllist,elonglist,initialtracked,parameterlist,masterprefix,testprefix,trials,max_strand_nr,maxStrandLength,numCells,numRounds,motif,elong,bias):
	pop_tracker = []
	elongation_tracker = []
	all_tracked = []
	...
	with open(masterprefix+ testprefix +'_MotifData_motif{motif}_len{maxStrandLength}_bias{bias}_elong{elong}_{trials}trials_numRound{numRounds}.csv'.format(motif = motif, maxStrandLength = maxStrandLength, bias=bias, elong=elong, trials=trials, numRounds=numRounds), 'wb') as f: 
		...
		for trial in xrange(trials):
			...
			all_tracked.append({})
			nr_motifs, nr_strands, nr_cells_with_motif, pop_tracker[trial], elongation_tracker[trial], all_tracked[trial]  = motifsimtracker_trial(deepcopy(celllist),deepcopy(elonglist),initialtracked,motif,max_strand_nr,maxStrandLength,numCells,numRounds,elong,bias)
	...
	return pop_tracker, nr_strands_per_time, elongation_tracker, all_tracked
